...
contract TestModuleManager_EnableMode is Test, TestModuleManagement_Base {
    ...
    function test_EnableMode_Uninitialized_7702_Account() public {
        ...
        assertEq(counter.getNumber(), counterBefore + 1, "Counter should have been incremented after single execution");
        assertTrue(
            IModuleManager(BOB_ADDRESS).isModuleInstalled(MODULE_TYPE_VALIDATOR, address(mockMultiModule), ""), "Module should be installed as validator"
        );
        assertTrue(IModuleManager(BOB_ADDRESS).isModuleInstalled(MODULE_TYPE_EXECUTOR, address(mockMultiModule), ""), "Module should be installed as executor");
    }
